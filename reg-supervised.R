...
# export mod.id
write.csv(mod.id, "reg-model-ID-numbers.csv", row.names = FALSE)
...
reg.pred = function(Xtrain, Ytrain, Xtest, Ytest, Y.type, cases, num_class)
{
  ...
  } else
  {
    ...
    # build the training model
    set.seed(42)
    mod = multinom(y ~ ., data = dat)
    
    # make predictions with the training model using the test set
    ypred = predict(mod, newdata = data.table(Xtest), type = "probs")
...
reg.cv = foreach(i = 1:tasks) %fun%
{
  ...
  output = reg.pred(Xtrain = Xtrain, Ytrain = Ytrain, Xtest = Xtest, Ytest = Ytest, 
                    num_class = num_class, Y.type = Y.type, cases = cases)
...
# export reg.cv
write.csv(reg.cv, "reg-cross-validation-performance.csv", row.names = FALSE)
...
if(Y.type == "Binary")
{
  ...
  reg.diag = reg.cv[,.(stat = factor(stat, levels = stat),
...
# add a mod and a rank column to reg.diag
reg.diag[, mod := 1]
reg.diag[, rank := 1]

# export reg.diag
write.csv(reg.diag, "reg-summary.csv", row.names = FALSE)

# add rank to reg.cv
reg.cv[, rank := 1]

# add a Model identity column
reg.cv[, Model := factor(ifelse(rank == 1, "Yes", "No"), levels = c("Yes", "No"))]

# plot our models performance metrics
# decide which type of model to run
if(Y.type == "Binary")
{
  reg.plot = ggpairs(reg.cv[,.(Accuracy, AUC, Log.Loss, Model)],
...
{
  reg.plot = ggpairs(reg.cv[,.(Rsquared, RMSE, MAPE, Model)],
                     mapping = aes(color = Model, fill = Model),
                     upper = list(continuous = wrap("density", alpha = 1/4), combo = "box"),
                     lower = list(continuous = wrap("points"), combo = wrap("dot")),
                     diag = list(continuous = wrap("densityDiag", alpha = 1/3))) + 
...
{
  reg.plot = ggpairs(reg.cv[,.(Macro.Accuracy, Micro.Accuracy, Multi.Log.Loss, Model)],
                     mapping = aes(color = Model, fill = Model),
                     upper = list(continuous = wrap("density", alpha = 1/4), combo = "box"),
                     lower = list(continuous = wrap("points"), combo = wrap("dot")),
                     diag = list(continuous = wrap("densityDiag", alpha = 1/3))) + 
...
pdf("reg-plot.pdf", width = 16, height = 10, paper = "special") 
...
if(Y.type == "Binary")
{
  ...
  # make predictions with the training model using the test set
  ynew = predict(mod, data.table(test), type = "response")
  ...
  dat2[, y := factor(as.numeric(ynew >= reg.diag[mod == 1 & stat == "Median", Cutoff]), levels = 0:1)]
  dat = rbind(dat, dat2)
  ...
  ynew = as.numeric(ynew >= reg.diag[mod == 1 & stat == "Median", Cutoff])
...
} else if(Y.type == "Numeric")
{
  ...
  # make predictions with the training model using the test set
  ynew = as.numeric(predict(mod, data.table(test)))
...
{
  # set our reference level for which a coefficient won't be made
  Y = relevel(Y, ref = "0")
  ...
  # make predictions with the training model using the test set
  ynew = predict(mod, newdata = data.table(test), type = "probs")
...
# export test
write.csv(test, "reg-test-predictions.csv", row.names = FALSE)

# export var.data
file.create("reg-variable-data.txt")
sink("reg-variable-data.txt")
print(summary(mod))
sink()
